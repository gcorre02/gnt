...
{
    /// <summary>
    /// To localise the entry level of the ROC with respect to its extremes.
    /// </summary>
    [Description("To localise the entry level of the ROC with respect to its extremes.")]
    public class ROC_ret_v1 : Strategy
    {
        #region Variables
        // Wizard generated variables

        private double entryLevelDif = 0.01; // Default setting for EntryLevelDif
        private int stopLoss = 7; // Default setting for StopLoss
        private int profit = 15; // Default setting for Profit
        private int rOCPeriod = 14; // Default setting for RSIPeriod
		...
		private int priceDif = 10;
		private int isPositiveOn = 1; 
		private double rOCoBought = 0.05;
		private double rOCoSold = 0.05;
        ...
        protected override void OnBarUpdate()
        {
			...
			if(CurrentBar < ROCPeriod || itsOn == 0){
				return;
			}
			...
			if(ROC(ROCPeriod)[0]<=(-ROCoSold)){
				belowLimit = ROC(ROCPeriod)[1];
        	    if (CrossAbove(ROC(ROCPeriod), belowLimit, 1) 
					&& (ROC(ROCPeriod)[0]-belowLimit>=entryLevelDif )
			...
			if(ROC(ROCPeriod)[0]>=(ROCoBought)){
				aboveLimit = ROC(ROCPeriod)[1];         
            	if (CrossBelow(ROC(ROCPeriod), aboveLimit, 1)
					&& ( aboveLimit-ROC(ROCPeriod)[0]>=entryLevelDif) 
		...
		[Description("")]
        [GridCategory("Parameters")]
        public double ROCoBought
        {
            get { return rOCoBought; }
            set { rOCoBought = Math.Max(0, value); }
        }
        [Description("")]
        [GridCategory("Parameters")]
        public double ROCoSold
        {
            get { return rOCoSold; }
            set { rOCoSold = Math.Max(0, value); }
        }
        [Description("")]
        [GridCategory("Parameters")]
