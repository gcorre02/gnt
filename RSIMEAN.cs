...
namespace NinjaTrader.Strategy
{
    ...
    public class RSIMEAN : Strategy
    {
        ...
        {
			currentPnL = Positions[0].GetProfitLoss(Close[0],PerformanceUnit.Points)/0.01;
            // Condition set 1
            if (CrossBelow(RSI(RsiBars, 3), OverBoughtRsi, 1)&& Positions[0].Quantity ==0)
            {
				EnterShortLimit(PositionSize,Close[0]+0.01*10, "");
				EnterShort(PositionSize);
                //EnterShortLimit(PositionSize, Close[0], "");
            }
		//	Log("AddOnOff "+ AddOnOff +"current direction" + Positions[0].MarketPosition.ToString() +" current PnL" + currentPnL +"against " + (-AddAtTicks)+ "current averagePrice is " + Positions[0].AvgPrice + "current quantity is " + Positions[0].Quantity,LogLevel.Information);  
            // Condition set 2
            //if ((-AddAtTicks) > currentPnL  
            ...
            {
				EnterLongLimit(PositionSize, Close[0]-0.01*10, "");
				EnterLong(PositionSize);
//                EnterLongLimit(PositionSize, Close[0], "");
            }
